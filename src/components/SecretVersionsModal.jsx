        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <code className="px-2 py-1 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded font-mono text-sm">
                      {secret.name}
                    </code>
                    {enabledVersion && (
                  ...
                  {secret.labels && Object.keys(secret.labels).length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {Object.entries(secret.labels).map(([key, value]) => (
                        <span
                          key={key}
                          className="inline-flex items-center gap-1 px-2 py-1 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md text-xs"
                        >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-2 bg-amber-500 hover:bg-amber-600 text-white font-medium rounded-lg transition-all text-sm"
                >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {version.status === "enabled" && (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full text-xs font-medium">
                                <CheckCircle className="w-3 h-3" />
                                Active
                              </span>
                            )}
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <ul className="text-xs text-amber-800 dark:text-amber-200 space-y-1">
                    <li>
                      • Secret values are encrypted server-side and never
                      exposed
                    </li>
                    <li>
                      • Only the active version is used by service accounts
                    </li>
                    <li>• Disabled versions are kept for audit and rollback</li>
                    <li>• All access is logged for security compliance</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex-shrink-0">
            <button
              onClick={handleClose}
              disabled={!!isUpdating}
              className="w-full px-4 py-2.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-semibold hover:bg-slate-300 dark:hover:bg-slate-600 transition-all disabled:opacity-50"
            >
              Close
            </button>
          </div>
